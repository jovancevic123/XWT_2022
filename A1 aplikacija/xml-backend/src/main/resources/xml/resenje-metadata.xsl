<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="a1:resenje">
        <resenje vocab="http://www.ftn.uns.ac.rs/rdf/examples/predicate/"
                about="http://www.ftn.uns.ac.rs/xwt/resenje/{./a1:broj_resenja}">
            <xsl:apply-templates select="node()|@*"/>
        </resenje>
    </xsl:template>

    <xsl:template match="a1:resenje/a1:broj_resenja">
        <BrojResenja property="pred:broj_resenja" datatype="xs:string">
            <xsl:apply-templates select="node()|@*"/>
        </BrojResenja>
    </xsl:template>

    <xsl:template match="a1:resenje/a1:datum_odgovora">
        <DatumOdgovora property="pred:datum_odgovora" datatype="xs:date">
            <xsl:apply-templates select="node()|@*"/>
        </DatumOdgovora>
    </xsl:template>

    <xsl:template match="a1:resenje/a1:prihvacena">
